...
test("pubsub", async () => {
    ...
    const yamlFilePath = path.join(__dirname, '../', '../', 'example', 'experimental', 'workflow', 'pubsub.yaml');
    // const yamlFilePath = path.join(path.dirname(fileURLToPath(import.meta.url)), '../', '../', 'example', 'experimental , 'workflow',pubsub.yaml');
...
test("correlate", async () => {
    ...
    // Load the YAML from the file
    const yamlFilePath = path.join(__dirname, '../', '../', 'example', 'experimental', 'workflow', 'correlate.yaml');
    const templateYaml = fs.readFileSync(yamlFilePath, 'utf8');
    var template = yaml.load(templateYaml);
    const tp = StatedWorkflow.newWorkflow(template);
    await tp.initialize();
...
test("workflow logs", async () => {
    ...
    // Load the YAML from the file
    const yamlFilePath = path.join(__dirname, '../','../','example', 'experimental', 'workflow', 'wf.yaml');
    const templateYaml = fs.readFileSync(yamlFilePath, 'utf8');
...
test("workflow perf", async () => {
    ...
    // Load the YAML from the file
    const yamlFilePath = path.join(__dirname, '../', '../', 'example', 'experimental', 'workflow', 'wfPerf01.yaml');
...
test("webserver", async () => {
    ...
    // Load the YAML from the file
    const yamlFilePath = path.join(__dirname, '../', '../', 'example', 'experimental', 'workflow', 'wfHttp01.yaml');
    console.time("Read YAML file"); // Start the timer for reading the file
    const templateYaml = fs.readFileSync(yamlFilePath, 'utf8');
    console.timeEnd("Read YAML file"); // End the timer for reading the file
...
test("500 parallel downloaders", async () => {
    console.time("workflow perf total time"); // Start the timer with a label

    // Load the YAML from the file
    const yamlFilePath = path.join(__dirname, '../', '../', 'example', 'experimental', 'workflow', 'wfDownloads.yaml');
    console.time("Read YAML file"); // Start the timer for reading the file
    const templateYaml = fs.readFileSync(yamlFilePath, 'utf8');
    console.timeEnd("Read YAML file"); // End the timer for reading the file
